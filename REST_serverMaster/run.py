from hazards_registry import app, server_port,q_hazard, downlink_port
import queue
...
print ("serving at ports: ", server_port, " uplink", downlink_port, " downlink")
...
if __name__ == "__main__":
    try:
        s=ThreadedServer('0.0.0.0', downlink_port, timeout=86400, debug=False)
        s.daemon=True
        s.start()

    #ThreadedServer('127.0.0.1', 8008, timeout=86400, callback=some_callback, debug=True).start() #to enable a callback
        app.run(host='0.0.0.0', port=server_port, debug=False,use_reloader=False)
    #app.run(host='0.0.0.0', port=15080, debug=True,use_reloader=False)
